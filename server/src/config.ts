...
function resolveConfigPath(): string {
  const envPath = process.env.MASQUERADARR_CONFIG;
  if (envPath) {
    if (!existsSync(envPath)) {
      throw new Error(`MASQUERADARR_CONFIG points to "${envPath}" but the file does not exist.`);
    }
    return envPath;
  }
  ...
  throw new Error(
    'No config file found. Set MASQUERADARR_CONFIG to the mounted config path, ' +
